val sonatypeCredentials = for {
	username <- Option(sys.props("SONATYPE_USERNAME")).orElse(Option(sys.env("SONATYPE_USERNAME")))
	password <- Option(sys.props("SONATYPE_PASSWORD")).orElse(Option(sys.env("SONATYPE_PASSWORD")))
