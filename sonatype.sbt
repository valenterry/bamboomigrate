val sonatypeCredentials = for {
	username <- Option(sys.props("SONATYPE_USERNAME")).flatMap(sys.env("SONATYPE_USERNAME"))
	password <- Option(sys.props("SONATYPE_PASSWORD")).flatMap(sys.env("SONATYPE_PASSWORD"))
